...
def compress(psi, L, chi_max):
    """
    Compress an wave vector psi to a left-canonical MPS with bond dimension chi_max.
    """
    ...
    for n in range(1, L+1):
        ...
        assert dim_R == 2**(L-(n-1)), f"Dimension mismatch: dim_R = {dim_R}, 2**(L-n) = {2**(L-n)}"
...
def reconstruct_psi_from_mps(Ms):
    ...
    for M in Ms[1:]:
        # Contract along virtual bond index
        psi = np.tensordot(psi, M, axes=([psi.ndim-1], [0]))
